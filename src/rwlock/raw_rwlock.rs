...
const WRITER_BIT: u32 = 0b1000;
// Mask of bits used to count readers.
const READERS_MASK: u32 = !0b1111;
// Base unit for counting readers.
const ONE_READER: u32 = 0b10000;

// Token indicating what type of lock a queued thread is trying to acquire
const TOKEN_SHARED: ParkToken = ParkToken(ONE_READER as usize);
const TOKEN_EXCLUSIVE: ParkToken = ParkToken(WRITER_BIT as usize);
const TOKEN_UPGRADABLE: ParkToken = ParkToken((ONE_READER | UPGRADABLE_BIT) as usize);
...
unsafe impl lock_api::RawRwLockRecursive for RawRwLock {
    ...
    fn lock_shared_recursive(&self) {
        if !self.try_lock_shared_fast(true) {
            let result = self.lock_shared_slow(true);
            debug_assert!(result);
        }
        self.deadlock_acquire();
    }
...
unsafe impl lock_api::RawRwLockUpgrade for RawRwLock {
    ...
    fn lock_upgradable(&self) {
        if !self.try_lock_upgradable_fast() {
            let result = self.lock_upgradable_slow();
            debug_assert!(result);
        }
        self.deadlock_acquire();
    }
...
impl RawRwLock {
    ...
    fn lock_exclusive_slow(&self) -> bool {
        ...
        let timed_out = !self.lock_common(TOKEN_EXCLUSIVE, try_lock, WRITER_BIT | UPGRADABLE_BIT);
        if timed_out {
            return false;
        }
    ...
    #[cold]
    fn lock_upgradable_slow(&self) -> bool {
        let try_lock = |state: &mut u32| {
            let mut spinwait_shared = SpinWait::new();
            loop {
                if *state & (WRITER_BIT | UPGRADABLE_BIT) != 0 {
        ...
        self.lock_common(TOKEN_UPGRADABLE, try_lock, WRITER_BIT | UPGRADABLE_BIT)
    }
    ...
    fn upgrade_slow(&self) -> bool {
        self.wait_for_readers(ONE_READER | UPGRADABLE_BIT)
    }
    ...
    unsafe fn wake_parked_threads(
        &self,
        new_state: u32,
        callback: impl FnOnce(u32, UnparkResult) -> UnparkToken,
    ) {
    ...
    // WRITER_BIT.
    #[inline]
    fn wait_for_readers(&self, prev_value: u32) -> bool {
    ...
    fn lock_common(
        &self,
        token: ParkToken,
        mut try_lock: impl FnMut(&mut u32) -> bool,
        validate_flags: u32,
    ) -> bool {
        let mut spinwait = SpinWait::new();
        let mut state = self.state.load(Ordering::Relaxed);
